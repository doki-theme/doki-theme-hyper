  ...
  return toPairs(DEFAULT_TERMINAL_COLORS)
    .map(([colorName, colorValue]) => {
      const cleanedName = startsWith(colorName, "light")
        ? colorName.substr(5).toLowerCase()
        : colorName;
      const themeTerminalColorAttribute =
        DOKI_TO_HYPER_COLOR_MAPPINGS[cleanedName];
      const themeTerminalColor = themeTerminalColorAttribute
        ? dokiTheme.colors[themeTerminalColorAttribute] || colorValue
        : colorValue;
      return [colorName, themeTerminalColor];
    })
    .reduce(
      (accum: StringDictonary<string>, [colorName, themeTerminalColor]) => {
